...
const insert = async (req, res) => {
  try {
    ...
    if (
      ...
      var values = [
        [index, "ssc", "10th", data.sboard, data.syear, data.sper],
    ...
    if (
      typeof data.gujarati == "undefined" &&
      typeof data.hindi == "undefined" &&
      typeof data.english == "undefined"
    ) {
      var e = "internal server error...";
      res.render("insertejs/error", { e });
    }
    ...
    if (
      typeof data.larvel == "undefined" &&
      typeof data.php == "undefined" &&
      typeof data.oracle == "undefined" &&
      typeof data.mysql == "undefined"
    ) {
...
const update = async (req, res) => {
  try {
    ...
    if (
      ...
      var values = [
        ["10th", data.sboard, data.syear, data.sper, id, "ssc"],
    ...
    let workUpdate = `update workExper set cname = ?,designation = ?,fromW = ?,toW = ? where Id = ?`;
    if (typeof(data.cname) == 'undefined') {
    } else {
      if(typeof(data.cname) == 'string'){
          let values = [data.cname,data.cdes,data.cfrom,data.cto,id];
          const res = await con.query(workUpdate,values);
      }
      else{
          data.cname.forEach(async (element,index) => {
              let values  = [data.cname[index],data.cdes[index],data.cfrom[index],data.cto[index],id];
...
// language
    ...
    lan.forEach(async (element, ind) => {
      if (typeof element != "undefined") {
        ...
        let update = `update languageKnown set lread = ?,lwrite = ?,speak = ? where language = ? and Id = ?`;
        let value = ['n', 'n', 'n', language[ind], id];
        await con.query(update, value);
    }
    });
      ...
      if (typeof data.refname == "string") {
        ...
        for (var i = 0; i < data.refname.length; i++) {
          if (
            ...
            const reference = data.refname;
            Object.values(reference).forEach(async(e,index)=>{
              if (
                data.refname[index] == "" &&
                data.refcon[index] == "" &&
                data.refrel[index] == ""
                ...
                console.log(values)
                const result = await con.query(refUpdate, values);
              }
            });
          }
        }
